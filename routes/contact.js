...
router.get("/mycontacts", auth, async (req, res) => {
  try {
    ...
    return res.status(200).json({ contacts: myContacts.reverse() });//reverse is used here to get the latest contact first in the list
  } catch (err) {
    console.log(err);
  }
});
...
router.put("/contact", auth, async (req, res) => {
  ...
  try {
    ...
    const updatedData = { ...req.body, id: undefined };// id is undefined here not to update id
    const result = await Contact.findByIdAndUpdate(id, updatedData, {
      new: true,
    });// to get the updated data immediatly we are using new:true here
